...
_CACHE_TTL = 120.0
_LIST_CACHE_TTL = 45.0
_list_cache: dict[str, tuple[float, list[Conversation]]] = {}
...
def _list_cache_key(owner_id: Optional[int], day: Optional[str],
                    user_id: Optional[int], query: str) -> str:
    return f"{owner_id}|{day or ''}|{user_id}|{query.strip().lower()}"


async def _build_conversation_list(
    query: str = "",
    day: Optional[str] = None,
    owner_id: Optional[int] = None,
    user_id: Optional[int] = None,
) -> list[Conversation]:
    """Lista enriquecida (modo smart), sem filtro de ação — base para cache."""
    if settings.mock:
        _load_mock_state()
    ...
    return sort_conversations(items, "smart")
...
async def get_enriched_list(
    ...
    *,
    ...
    """Lista smart em cache — compartilhada entre inbox e alertas."""
    ck = _list_cache_key(owner_id, day, user_id, query)
    now = time.monotonic()
    if use_cache:
        hit = _list_cache.get(ck)
        if hit and now - hit[0] < _LIST_CACHE_TTL:
            return hit[1]
    items = await _build_conversation_list(
        query=query, day=day, owner_id=owner_id, user_id=user_id,
    )
    _list_cache[ck] = (now, items)
    return items


async def warm_ploomes() -> None:
    """Pré-aquece Ploomes após login (estágios + amostra de negócios)."""
...
async def list_conversations(
    ...
    """Lista com cache compartilhado; paginação opcional (limit>0)."""
...
async def global_search(
    ...
    """Busca em conversas (cache) + texto de mensagens WhatsApp locais."""
    import db

    needle = (q or "").strip()
    if len(needle) < 2:
        return {"conversations": [], "messages": [], "q": needle}

    items, _ = await list_conversations(
        mode="smart", query=needle, owner_id=owner_id, user_id=user_id,
    )
    conv_hits = [
        {
            "id": c.id, "name": c.name, "phone": c.phone,
            "snippet": getattr(c, "last_preview", None) or c.stage or "",
            "stage": c.stage,
        }
        for c in items[:limit]
    ]
    # busca mais ampla na lista em cache
    if len(conv_hits) < limit:
        all_items = await get_enriched_list(owner_id=owner_id, user_id=user_id)
        nq = needle.lower()
        seen = {h["id"] for h in conv_hits}
        for c in all_items:
            if c.id in seen:
                continue
            blob = " ".join(filter(None, [
                c.name, c.contact, c.phone or "", c.cnpj or "", c.stage or "",
            ])).lower()
            if nq in blob:
                conv_hits.append({
                    "id": c.id, "name": c.name, "phone": c.phone,
                    "snippet": getattr(c, "last_preview", "") or c.stage or "",
                    "stage": c.stage,
                })
                seen.add(c.id)
            if len(conv_hits) >= limit:
    ...
    msg_hits = db.search_messages(needle, limit=min(20, limit))
    return {"conversations": conv_hits[:limit], "messages": msg_hits, "q": needle}
